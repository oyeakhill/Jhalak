...
if not os.path.exists('audios'):
    ...
    for each in LABELS:
        ...
        myobj.save("audios/"+text+".mp3")
        text = each + extra + straight
        myobj = gTTS(text=text, lang='en', slow=False)
        myobj.save("audios/"+text+".mp3")
        text = each + extra + rhs
        myobj = gTTS(text=text, lang='en', slow=False)
...
while True:
    ...
    if frame_count % 100 == 0:
        ...
        blob = cv2.dnn.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True,
                                     crop=False)  # You can use (288,288) to increase fps
        net.setInput(blob)
        ...
        if boxes:
            cv2.rectangle(frame, (boxes[-1][0], boxes[-1][1]), (boxes[-1][0] + boxes[-1][2], boxes[-1][1] + boxes[-1][3]), colour)
            cv2.putText(frame, LABELS[classIDs[-1]], (boxes[-1][0], boxes[-1][1]-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colour)
